(function ($) {
  ...
  function handleSticky() {
    var $sticky = $(".header-sticky");
    if (!$sticky || $sticky.length === 0) return;
    var scroll = windows.scrollTop();
    var headerHeight = $sticky.outerHeight() || 0;
    var screenSize = windows.width();
    // If header is absolute, show sticky earlier (small scroll), otherwise use header height
    var threshold = $sticky.hasClass('header_absolute') ? 10 : headerHeight;
  ...
  windows.on("scroll", handleSticky);
  windows.on("resize", handleSticky);
  // run when header partial is injected
  $(document).on('lm-header-init', handleSticky);
  // run once on load to set initial state
  handleSticky();
  // small fallback: re-run after a short delay in case header was injected after initial run
  setTimeout(handleSticky, 300);
  ...
  $(document).on("click", ".toolbar-btn", function (e) {
    ...
    // If href is empty or '#', allow data-toggle to map to a target id
    if (!target || target === "#") {
      var toggle = $this.data('toggle');
      if (toggle) {
        if (toggle === 'minicart' || toggle === 'miniCart') target = '#miniCart';
        else if (toggle === 'search-overlay') target = '#search-overlay';
        else if (toggle === 'mobile-menu') target = '#mobile-menu-overlay';
        else target = '#' + toggle;
      }
    }
    var prevTarget = $this
  ...
  /*  Close Button Actions
    /*----------------------------------------*/
  $(document).on("click", ".btn-close, .btn-close-2, body .global-overlay", function (e) {
  ...
  /*=============================================
    ...
    =============================================*/
  // Offcanvas navigation submenu prep (ensure menu-expand exists after injection)
  $(document).on('lm-offcanvas-init', function () {
    var $offCanvasNavSubMenu = $('.offcanvas-navigation').find('.sub-menu');
    $offCanvasNavSubMenu.parent().prepend('<span class="menu-expand"><i></i></span>');
    $offCanvasNavSubMenu.slideUp();
  });

  // Delegated category submenu toggle
  $(document).on("click", ".offcanvas-navigation li a, .offcanvas-navigation li .menu-expand", function (e) {
    var $this = $(this);
    if (
      $this
        .parent()
  ...
  // Mobile menu triggers (delegated)
  $(document).on("click", "#mobile-menu-trigger, #mobile-menu-trigger-2", function (e) {
    e.preventDefault();
    $("#mobile-menu-overlay").addClass("active");
    $body.addClass("no-overflow");
  });

  $(document).on("click", "#mobile-menu-close-trigger", function (e) {
    e.preventDefault();
  ...
  // Search overlay triggers (delegated)
  $(document).on("click", "#search-overlay-trigger", function (e) {
    e.preventDefault();
    $("#search-overlay").addClass("active");
    $body.addClass("no-overflow");
  });

  $(document).on("click", "#search-close-trigger", function (e) {
    e.preventDefault();
